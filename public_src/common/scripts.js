...
const doubleEncrypt = async (encryptedValue, keyUrls) => {
	...
	const generatedKeys = await window.jscec.generateKey('P-256');
	console.log(generatedKeys.privateKey);
	console.log('privateKeyData.randomData ', privateKeyData.randomData);
    ...
    console.log(stringifiedSignature, ' : stringifiedSignature');
    return btoa(encryptedValue) + '-data-' + stringifiedSignature;
};
