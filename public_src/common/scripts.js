...
const doubleEncrypt = async (encryptedValue, keyUrls) => {
	...
	const now = new Date();
	const currentTimeStamp = '' + now.getUTCFullYear() + now.getUTCMonth() + now.getUTCDate() + now.getUTCDay() + now.getUTCHours() + now.getUTCMinutes() + now.getUTCSeconds() + now.getUTCMilliseconds();

    const message =  new TextEncoder().encode(encryptedValue + '----' +currentTimeStamp);
